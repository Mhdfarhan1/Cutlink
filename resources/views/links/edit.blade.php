<x-layouts.app>
    <x-slot:title>Edit Link — {{ $shortLink->name }}</x-slot:title>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2 text-xs font-semibold text-blue-600 mb-1">
            ...
            <span>/</span>
            <span class="text-slate-400">Edit Tautan</span>
        </div>
        <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Edit Short Link</h1>
        <p class="text-xs sm:text-sm text-slate-500 mt-1">
            Perbarui informasi URL tujuan, alias, atau pengaturan bridge page.
        </p>
    </div>

    <!-- Container Form -->
    <div class="max-w-3xl" x-data="{
        alias: '{{ old('code', $shortLink->code) }}',
        baseUrl: '{{ url('/') }}/',
        bridgeEnabled: {{ old('bridge_enabled', $shortLink->bridge_enabled ? '1' : '0') === '1' ? 'true' : 'false' }},
        qrEnabled: {{ old('qr_enabled', $shortLink->qr_enabled ? '1' : '0') === '1' ? 'true' : 'false' }},
        isActive: {{ old('is_active', $shortLink->is_active ? '1' : '0') === '1' ? 'true' : 'false' }}
    }">
        <div class="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-xs">
            
            <form method="POST" action="{{ route('links.update', $shortLink) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            value="{{ old('destination_url', $shortLink->destination_url) }}"
                            required
                ...
                <div>
                    <label for="code" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                        Alias / Kode Tautan <span class="text-rose-500">*</span>
                    </label>
                    ...
                    <div class="mt-3 p-3 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-between text-xs">
                        <span class="text-slate-600 font-medium">Short URL:</span>
                        <span class="font-mono font-bold text-blue-700" x-text="baseUrl + (alias.trim() ? alias.trim() : '...')"></span>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100 space-y-4">
                    ...
                    <label class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition">
                        <div>
                            ...
                            <div class="text-[11px] text-slate-500">Tautan dapat langsung diakses publik.</div>
                        </div>
                    ...
                    <label class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition">
                        <div>
                            ...
                            <div class="text-[11px] text-slate-500">Tampilkan halaman perantara resmi PIK-R REQUEST dengan countdown 5 detik.</div>
                        </div>
                    ...
                    <label class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition">
                        <div>
                            ...
                            <div class="text-[11px] text-slate-500">Aktifkan pembuatan QR Code untuk tautan ini.</div>
                        </div>
